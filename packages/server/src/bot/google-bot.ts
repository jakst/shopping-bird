...
interface PPPage {
	readonly page: () => Effect.Effect<never, never, Page>
}

const PageContext = Context.Tag<PPPage>()

// An effect that removes an item at a given position. It takes a page as input.
function removeItemAtPosition2(page: Page, position: number) {
	return pipe(
		Effect.tryCatchPromise(
			async () => {
				const nameDisplay = (await page.$x(`//ul/li//div[@role="button"]`))[position] as ElementHandle<HTMLDivElement>
				const name = await nameDisplay.evaluate((el) => el.textContent)
				Effect.logInfo(`[BOT]: Removing item "${name}" at position ${position}`)
...
// const getNameDisplayAtPosition = async (position: number) => {
// 	try {
// 		const nameDisplay = (await page.$x(`//ul/li//div[@role="button"]`))[position] as ElementHandle<HTMLDivElement>
// 	} catch (e) {
...
// 	return resume(Effect.succeed(name))
// })
