	...
	for (let i = 0; i < 100; i++) {
		for (const c of [c1, c2]) {
			...
			if (Math.random() < 0.5) {
				...
				const randomItem = c.shoppingList.items[Math.floor(Math.random() * c.shoppingList.items.length)]
				...
				if (random < 1 / 5 || c.shoppingList.items.length < 1) {
					...
					// Delete item
					actionLog.push(`await ${cString}.client.deleteItem("${randomItem.id}");`)
					await c.client.deleteItem(randomItem.id)
				} else if (random < 3 / 5) {
					// Rename item
					const newName = createRandomString()
					actionLog.push(`await ${cString}.client.renameItem("${randomItem.id}", "${newName}");`)
					await c.client.renameItem(randomItem.id, newName)
				} else if (random < 4 / 5) {
					// Set item checked
					actionLog.push(`await ${cString}.client.setItemChecked("${randomItem.id}", ${!randomItem.checked});`)
			...
			for (let i = 0; i < numberOfActions; i++) {
				...
				if (random < 2 / 4) {
					...
					if (setup.externalList.length > 0) {
						...
						actionLog.push(`setup.externalList[${i}].checked = ${!item.checked};`)
