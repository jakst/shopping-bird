...
test("Applies events locally when not connected", async () => {
	...
	expect(c1.shoppingList.items).toEqual([expect.objectContaining({ name: "Ost", checked: false })])
...
describe("client.clearCheckedItems()", () => {
	test("Only clears checked items", async () => {
		...
		expect(shoppingList.items).toEqual([expect.objectContaining({ name: "Skinka", checked: false })])
	})
